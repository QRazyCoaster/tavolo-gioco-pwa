...
interface NarratorViewProps {
  ...
  onCorrectAnswer?: (playerId: string) => void;  // now optional
  onWrongAnswer?: (playerId: string) => void;    // now optional
  onNextQuestion: () => void;
  timeLeft: number;
...
const NarratorView: React.FC<NarratorViewProps> = ({
  ...
  if (!currentPlayerAnswering || !playerInfo) {
    // No one in queue → show rankings
